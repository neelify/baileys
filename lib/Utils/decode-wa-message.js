...
const BAD_MAC_ERROR_TEXT = 'Bad MAC'

const normalizeDecryptErrorMessage = (error) => {
    const message = String(error?.message || error || '').trim()
    if (!message) {
        return message
    }
    if (/key used already or never filled|received message with old counter|chain closed|invalid prekey id|missing signed\s*prekey|no senderkeyrecord found for decryption|no session found to decrypt message/i.test(message)) {
        return MISSING_KEYS_ERROR_TEXT
    }
    if (/bad mac|invalidmessageexception/i.test(message)) {
        return BAD_MAC_ERROR_TEXT
    }
    return message
}
            ...
            if (Array.isArray(stanza.content)) {
                for (const { tag, attrs, content } of stanza.content) {
                    ...
                    try {
                        ...
                        fullMessage.messageStubParameters = [normalizeDecryptErrorMessage(err)]
                    }
                }
            }
