      ...
      await Promise.all([
        processingMutex.mutex(async () => {
          ...
          if (
            ...
            const decryptFailureText = String(msg?.messageStubParameters?.[0] || "");
            const isMissingKeysFailure =
              decryptFailureText === Utils_1.MISSING_KEYS_ERROR_TEXT ||
              /received message with old counter|chain closed|invalid prekey id|missing signed\s*prekey|no senderkeyrecord found for decryption|no session found to decrypt message/i.test(
                decryptFailureText
              );
            const isBadMacFailure =
              decryptFailureText === Utils_1.BAD_MAC_ERROR_TEXT ||
              /invalidmessageexception/i.test(decryptFailureText);
            if (
              isMissingKeysFailure ||
              isBadMacFailure
            ) {
              const nackReason = isBadMacFailure
                ? Utils_1.NACK_REASONS.SignalErrorOldCounter
                : Utils_1.NACK_REASONS.ParsingError;
              return sendMessageAck(node, nackReason);
            }
